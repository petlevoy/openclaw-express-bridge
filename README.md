# openclaw-express-bridge 1.1.11
...
```bash
sudo apt install ./openclaw-express-bridge_1.1.11_amd64.deb
...
```bash
tar -xzf openclaw-express-bridge-1.1.11-linux-amd64.tar.gz
cd openclaw-express-bridge-1.1.11
./install.sh
...
Plugin 2.3.3 supports `desktopChats`, an exact array allowlist that takes precedence
